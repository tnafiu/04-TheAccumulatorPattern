...
def run_test_sum_more_cosines():
    ...
    # ------------------------------------------------------------------
    # Test 2:
    expected = -1.2358  # This is APPROXIMATELY the correct answer.
    answer = sum_more_cosines(1, 5)
    print('Test 2 expected:', expected, '(approximately)')
    print('       actual:  ', answer)

    # Test 3:
    expected = 1.1242  # This is APPROXIMATELY the correct answer.
...
def count_sines_from(m, n):
    ...
    for k in range(n - m + 1):  # Loop
        if math.sin(k + m) < 0.5:  # If the condition holds:
...
def count_sines_vs_cosines(m):
    ...
    import math
    count = 0  # Initialize to 0 BEFORE the loop
    for k in range((2 * m) + 1):  # Loop
        if math.sin(k - m) > math.cos(k - m):  # If the condition holds:
            count = count + 1  # Increment INSIDE the loop.
    return count
